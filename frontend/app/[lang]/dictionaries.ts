...
function fillWithKey(primary: any, reference: any, path: string[] = []): any {
  ...
  for (const key of Object.keys(reference)) {
    ...
    result[key] = fillWithKey(primary?.[key], reference[key], [...path, key])
  }
...
export const getDictionary = async (locale: 'en' | 'fr') => {
  ...
  const safeLocale: SupportedLocale =
    locale in dictionaries ? (locale as SupportedLocale) : 'en'
