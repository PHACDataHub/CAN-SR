...
import {
  ...
  type ExtractionParameter,
} from '@/components/can-sr/extraction-parameters'
...
export default function CanSrL2ScreenPage() {
  ...
  type ParametersParsed = { items: ExtractionParameter[] }
  ...
  const [parametersParsed, setParametersParsed] =
    useState<ParametersParsed | null>(null)
  ...
  const [aiStatus, setAiStatus] = useState<
    Record<
      string,
      'queued' | 'extracting' | 'suggesting' | 'suggested' | 'error'
    >
  >({})
  ...
  const suggestParam = async (
    ...
    const runContext =
      context ||
      (citationId && srId ? { srId, citationId, model: selectedModel } : null)
    ...
    if (currentCitationKeyRef.current !== requestKey)
      return { ok: false, error: 'Citation changed' }
    setAiStatus((prev) => ({ ...prev, [name]: 'extracting' }))
    try {
      const headers = getAuthHeaders()
      const fullText =
        context && 'fullText' in context
          ? context.fullText
          : await ensureFullText()
      if (!fullText)
        throw new Error('Full text is not available for this citation')
      if (currentCitationKeyRef.current !== requestKey)
        return { ok: false, error: 'Citation changed' }
  ...
  const runAllAI = async () => {
    ...
    const params = parametersParsed.items
    ...
    try {
      ...
      for (let idx = 0; idx < params.length; idx++) {
        ...
        if (!result.ok && result.error !== 'Citation changed')
          failures.push(`${p.name}: ${result.error || 'failed'}`)
        if (currentCitationKeyRef.current !== requestKey) break
        setRunAllProgress({ done: idx + 1, total: params.length })
      }
      if (failures.length) {
        setRunAllError(
          `${failures.length} of ${params.length} parameters failed. ${failures[0]}`,
        )
      }
    } catch (err: any) {
      if (currentCitationKeyRef.current === requestKey)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (paramsInfo && (paramsInfo.items || paramsInfo.categories)) {
          const items = flattenExtractionParameters(paramsInfo)
          setParametersParsed({ items })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        parametersParsed.items.forEach(({ name }) => {
          ...
          <aside className="col-span-3">
            <div className="flex h-full flex-col space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              <div>
                ...
                {runAllError ? (
                  <div className="mt-1 text-xs text-red-600">{runAllError}</div>
                ) : null}
                ...
                {parametersParsed ? (
                  <div className="space-y-4">
                    <div className="space-y-3">
                      {parametersParsed.items.map((parameter) => {
                        const {
                          name: paramName,
                          ...
                          >
                            <div className="flex items-start justify-between">
                              <div className="flex items-center gap-1 text-xs font-medium text-gray-700">
                                ...
                                {cleanDesc ? (
                                  <button
                                    ...
                                    aria-label={
                                      descOpen[paramName]
                                        ? dict.extract.hideDescription
                                        : dict.extract.showDescription
                                    }
                                    title={
                                      descOpen[paramName]
                                        ? dict.extract.hideDescription
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  onClick={() =>
                                    suggestParam(paramName, cleanDesc)
                                  }
                                  disabled={
                                    runningAllAI ||
                                    aiStatus[paramName] === 'extracting' ||
                            ...
                            {options.length ? (
                              <fieldset className="mt-2 space-y-1">
                                <legend className="sr-only">{paramName}</legend>
                                {options.map((option) => (
                                  <label
                                    key={option}
                                  ...
                                  >
                                    <input
                                      ...
                                      value={option}
                                      checked={
                                        paramValues[paramName] === option
                                      }
                                      onChange={() =>
                                        updateValue(paramName, option)
                                      }
                                    />
                                    {option}
                                  </label>
