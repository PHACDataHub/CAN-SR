  ...
  try {
    ...
    if (documentIdParam) {
      ...
      if (!citationRes.ok) {
        ...
        const text = await citationRes.text().catch(() => null)
        console.error('Failed to fetch citation from backend', {
          url: citationUrl,
      ...
      // Read citation body (use text then try to parse to give better debug info)
      const citationText = await citationRes.text().catch(() => null)
      let citation: any = {}
      try {
        citation = citationText ? JSON.parse(citationText) : {}
      } catch {
        citation = { raw: citationText }
      }
    ...
    console.debug('Proxying file fetch', { fileFetchUrl, fetchOptions })
    ...
    if (!backendRes.ok) {
      ...
      console.error('Backend download returned error', {
        fileFetchUrl,
        status: backendRes.status,
        body: bodyText,
      })

      // Attempt to preserve the backend content-type for easier inspection in the browser/network tab
