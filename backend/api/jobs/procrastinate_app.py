...
class _AzurePsycopgConnector(PsycopgConnector):
    ...
    # type: ignore[override]
    async def open_async(self, pool=None, **kwargs) -> None:
        if pool is None:
...
async def clear_pending_jobs(*, queues: list[str] | None = None) -> int:
...
async def cancel_enqueued_jobs_for_run_all(job_id: str, *, queues: list[str] | None = None) -> int:
...
async def run_worker_once(*, queues: list[str] | None = None) -> None:
